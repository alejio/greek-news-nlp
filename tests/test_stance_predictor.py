...
def test_classify_article_positive(mock_openai_client):
    ...
    stance, justification = classify_article_with_explanation(
        mock_openai_client, article_text, "Ολυμπιακός"
    )
...
@pytest.mark.parametrize(
    "target_type,article_text,expected_stance",
    [
...
def test_classify_article_variations(
    mock_openai_client, target_type, article_text, expected_stance
):
    mock_openai_client.chat.completions.create.return_value.choices[
        0
    ].message.content = f"{expected_stance}\nTest explanation"

    stance, justification = classify_article_with_explanation(
        mock_openai_client, article_text, "Test Target", target_type
    )
